...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  devServer: {
    ...
    before(app) {
      let html = `<html><head><title>调试页</title>`
      html += `<style>body{margin: 0}.module-menu{float: left;width: 200px;height: 100%;padding: 0 8px;box-sizing: border-box}.module-container{float: left;width: calc(100% - 200px);height: 100%}.module-container iframe{width: 100%;height: 100%}</style>`
      html += `</head><body><div class="module-menu">`
      for (let module of moduleList) {
        html += `<a href="/${module}/index.html" target="container">${module.toString()}</a><br>`
      }
      html += `<p>当前正在进行多模块开发，点击链接切换模块</p>`
      html += `</div>`
...
module.exports = new Promise((resolve, reject) => {
  ...
  portfinder.getPort((err, port) => {
    if (err) {
      ...
      let urlInfos = [`Your application is running here: http://${baseUrl}`]
      if (moduleList && moduleList.length) {
        urlInfos = []
        for (let module of moduleList) {
          urlInfos.push(
            `module 【 ${module} 】 is running here: ${baseUrl}/${module}/index.html`
          )
        }
      }
